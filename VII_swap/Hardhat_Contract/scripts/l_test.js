...
// npx hardhat run scripts/l_test.js --network zhaomei
// npx hardhat run scripts/l_test.js --network bnbtest
// npx hardhat run scripts/l_test.js --network hardhat
...
async function main() {
    ...
    // return
    // 工厂合约
    contractname = "ViiderFactory";
    var viiderfactory = new ethers.Contract(
    ...
    // 路由合约
    contractname = "ViiderRouter";
    var viiderrouter = new ethers.Contract(
        contractinfo[network.config.chainId][contractname].address,
        contractinfo[network.config.chainId][contractname].abi,
        network.config.provider
    ).connect(owner);
    ...
    contractname = "WBNB";
    var wbnb = new ethers.Contract(
        contractinfo[network.config.chainId][contractname].address,
        contractinfo[network.config.chainId][contractname].abi,
        network.config.provider
    ).connect(owner);//和本币ETH/BNB/TBNB锚定，与本币一比一兑换，目的是将本币转换为erc20.
    contractname = "VIIDER";
    var viider = new ethers.Contract(
        contractinfo[network.config.chainId][contractname].address,
        contractinfo[network.config.chainId][contractname].abi,
        network.config.provider
    ).connect(owner);
    contractname = "Testtoken";
    var testtoken = new ethers.Contract(
        contractinfo[network.config.chainId][contractname].address,
        contractinfo[network.config.chainId][contractname].abi,
        network.config.provider
    ).connect(owner);
