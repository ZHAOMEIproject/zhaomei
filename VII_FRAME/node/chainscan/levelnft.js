...
function updatetokenidowner(addSqlParams){
    let updateSql = "replace into tokenowner(tokenid,owner,date,time) values(?,?,CURRENT_DATE()+0,unix_timestamp())";
    connection.replace(updateSql,addSqlParams);
};
// 添加铸造账号
function addtokenminter(addSqlParams){
    let updateSql = "replace into tokenminter(tokenid,owner,date,time) values(?,?,CURRENT_DATE()+0,unix_timestamp())";
    connection.replace(updateSql,addSqlParams);
};