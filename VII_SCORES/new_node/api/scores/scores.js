...
exports.getReleaseList = router.get("/update", async (req, res) => {
    ...
    +"total_score,date,lastranking,totaluser)"
    +"values(?,?,?,?,?,?,?,?,  ?,?,?,?,?,?,?,?,?,?,?,?,  ?,?,?,?, unix_timestamp(),?,?)";

    let time = Math.trunc(Date.now()/1000);
    let market_score = data.opensea_buy_s + data.opensea_gas_use_s + data.opensea_eth_use_s;
    let on_chain_score = data.success_nonce + data.main_nft + data.blue + data.superblue;
    let nft_score = data.total_nft;
    ...
    let detailed ={
        address:address,
        opensea_buy:data.opensea_buy,
        opensea_buy_s:data.opensea_buy_s,
        opensea_gas_use:data.opensea_gas_use,
        opensea_gas_use_s:data.opensea_gas_use_s,
        opensea_eth_use:data.opensea_eth_use,
        opensea_eth_use_s:data.opensea_eth_use_s,

        success_nonce:data.success_nonce,
        success_nonce_s:data.success_nonce_s,

        main_nft:data.main_nft,
        main_nft_s:data.main_nft_s,
        

        blue:data.blue,
        blue_s:data.blue_s,
        superblue:data.superblue,
        superblue_s:data.superblue_s,

        total_nft:data.total_nft,
        total_nft_s:data.total_nft_s,

        topaccount:data.topaccount,

        fistopenseatime:data.fistopenseatime,
        fist721time:data.fist721time,
